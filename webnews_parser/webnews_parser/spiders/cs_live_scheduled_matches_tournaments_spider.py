...
class CSlsMatchesTournamentsSpider(Spider):
    ...
    def start_requests(self):
        url_array = [
            "https://escorenews.com/en/csgo/matches?s1=" + str(page_num)
    ...
    def parse(self, response: Response, **kwargs: Any) -> Any:
        ...
        matches = (
            table_1.css("a.article.type1")
            + table_1.css("a.article")
            + table_1.css("a.article.v_gl704.type1")
            + table_1.css("a.article.type0.v_gl704")
        )
        for match in matches:
            scheduled_match_url = match.css("::attr(href)").get()
            scheduled_match_begin_time = css_mutator(
                "div.time i.sct::attr(datetime)", match
            )
            if scheduled_match_url.count("tbd") < 2:
                placeholder_dict = {
                    "match_begin_time": scheduled_match_begin_time,
                    "match_url": scheduled_match_url,
                }
                yield Request(
                    url=urljoin(
                        base="https://escorenews.com/", url=scheduled_match_url
                    ),
    ...
    def parse_match_page(self, response: Response, **kwargs: Any) -> Any:
        tournament_link = urljoin(
            base=self.base_url, url=response.xpath("//h1//a/@href").get()
        )
        tournament_info = xpath_mutator(
            '//div[contains(@class,"hh")]/span/text()', response
        ).split("•")
        tournament_format = (
            tournament_info[-2].strip() if tournament_info != [""] else ""
        )
        tournament_stage = (
            tournament_info[-3].strip() if tournament_info != [""] else ""
        )
        pretty_teams = response.xpath(
            '//div[contains(@class, "teams-on-live")]//h2/text()'
        ).getall()
        pretty_match_name = " - ".join([team.strip() for team in pretty_teams])
        teams = response.xpath(
            '//div[contains(@class, "teams-on-live")]/span/a/@href'
        ).getall()
        teams = [team.split("/")[-1] for team in teams]
        teams_logo_links = tuple(
            urljoin(base=self.base_url, url=_)
            for _ in response.xpath(
                '//div[contains(@class, "teams-on-live")]//picture//img/@src'
            ).getall()
        )
        teams_page_links = tuple(
            urljoin(base=self.base_url, url=_)
            for _ in response.xpath(
                '//div[contains(@class, "teams-on-live")]/span/a/@href'
            ).getall()
        ...
        if not teams:
            teams = ("", "")
        match_format = xpath_mutator(
            '//div[contains(@class, "score")]/h3/text()', response
        )
        match_format = match_format.strip()[match_format.find("B") :]
        match_status = response.xpath(
            '//div[contains(@class, "score")]//b/text()'
        ).get()
        if match_status == "Match did not start":
            match_status = "scheduled"
        elif match_status == "Match started":
            match_status = "live"
        match_score = response.xpath(
            '//div[contains(@class, "score")]/span[contains(@class, "live")]/text()'
        ).getall()
        if not match_score:
            match_score = ("", "")
        ...
        def TBD_team_page_boolean(team_pos: int) -> bool:
            ...
            return (
                teams_page_links[team_pos] != TBD_team_page_linkv1
        ...
        for stream_info in response.css("div.os-padding div.si"):
            ...
            stream_author_and_language = (
                css_mutator("u::text", stream_info).strip().split()
            )
            ...
            stream_link = (
                urljoin(base=twitch_url, url=streamer_twitch_name)
                if streamer_twitch_name
                else ""
            )
            match_streams[stream_name] = (
                stream_author,
                stream_language,
                stream_viewers,
                stream_link,
        ...
        placeholder_dict = {
            "match_info": {
                ...
                if TBD_team_page_boolean(0) or TBD_team_logo_boolean(0)
                else "TBD",
                "team1_logo_link": teams_logo_links[0]
                if TBD_team_logo_boolean(0) or TBD_team_page_boolean(0)
                else "TBD",
                ...
                if TBD_team_logo_boolean(1) or TBD_team_page_boolean(1)
                else "TBD",
                "team2_page_link": teams_page_links[-1]
                if TBD_team_page_boolean(1) or TBD_team_logo_boolean(1)
                else "TBD",
        ...
        if (
            placeholder_dict.get("match_info").get("team1") != "TBD"
            and placeholder_dict.get("match_info").get("team2") != "TBD"
        ):
    ...
    def parse_tournament_page(self, response: Response, **kwargs: Any) -> Any:
        ...
        bool_location_identifier = response.xpath(
            '//table[contains(@class, "tinfo table table-sm")]/tbody/tr[6]/td/span'
        ).get()
        tournament_name = response.css("div.hh h1::text").get()
        tournament_location = xpath_mutator(
            '//table[contains(@class, "tinfo table table-sm")]/tbody/tr[6]/td/text()',
            response,
        ).strip()
        tournament_logo_link = urljoin(
            self.base_url, response.css("div.tourlogo picture img::attr(src)").get()
        )
        tournmaent_description_raw = xpath_mutator_all(
            "//div[@class='tourdescription']//text()", response
        )
        ...
        tournament_start_date = xpath_mutator(
            '//table[contains(@class, "tinfo table table-sm")]'
        ...
        if not tournament_location:
            tournament_location_alt = xpath_mutator(
                "/html/body/div[1]/main/div[3]/div/div/div[1]/div/table/tbody/tr[5]/td/text()",
                response,
            ).strip()
        ...
        if not tournament_prize_pool:
            tournament_prize_pool = css_mutator(
                "body > div.wrap > main > div.page-topper > div > div > div.col-lg-4.order-last > div > table > tbody > tr:nth-child(3) > td::text",
                response,
            )
            tournament_location = css_mutator(
                "body > div.wrap > main > div.page-topper > div > div > div.col-lg-4.order-last > div > table > tbody > tr:nth-child(5) > td::text",
                response,
            )
        ...
        data_to_save = {
            ...
            if not bool_location_identifier
            else "",
            "tournament_logo_link": tournament_logo_link
            if tournament_logo_link != self.base_url
            else "",
