...
class CSUpdateLiveScheduledMatchesSpider(Spider):
    ...
    def parse_match(self, response, match_id, retry_times=0):
        ...
        if (not team_scores or not match_status) and retry_times < 3:
            ...
            return
        pretty_team_names = response.xpath(
        ...
        clean_match_format = (
            match_format.replace(",", "").strip() if match_format else "Best of 1"
        )

        team1_score = team_scores[0]
        team2_score = team_scores[1]
        tournament_url = urljoin(
            base="https://escorenews.com", url=response.xpath("//h1//a/@href").get()
        )
        team_urls = response.xpath(
            '//div[contains(@class, "teams-on-live")]/span/a/@href'
        ).getall()
        team1_url = (
            urljoin(base="https://escorenews.com", url=team_urls[0])
            if team_urls
            else None
        )
        team2_url = (
            urljoin(base="https://escorenews.com", url=team_urls[1])
            if len(team_urls) > 1
            else None
        )
        match_data = {
    ...
    def _parse_streams(self, response):
        ...
        for stream_info in response.css("div.os-padding div.si"):
            ...
            stream_link = (
                f"https://www.twitch.tv/{streamer_twitch_name}"
